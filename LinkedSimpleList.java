...
public class LinkedSimpleList<E> implements SimpleList<E> {
    ...
    //Returns the object if it is found in the list and brings the object to the front of the list. Returns null if the object is not found.
    public E findAndMoveFront(E x)
	{
		...
		while(curr != null)
		{
			...
			{
				//If the value to be found is already in the front
				if(curr.equals(head))
				{
					return x;
				}
				...
				{
					Node<E> pred = getNode(this.size - 2);
					pred.next = null;
					Node<E> nd = new Node<E>();
					nd.value = x;
					nd.next = this.head;
				...
				//If the value is anywhere but the start or the end
				else
				{
					Node<E> pred = getNode(i - 1);
					pred.next = pred.next.next;
